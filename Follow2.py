...
DEFAULT_SPEED = 75
ROTATION_FACTOR = 2
STOP_THRESHOLD = 100  # cm

# Konfigurasi performa real-time
CONTROL_FREQUENCY = 1000  # Hz - increased from 200
...
# OPTIMIZED SENSOR CONFIGURATION - Updated untuk posisi serong
SENSORS = {
    'front_left_diagonal': {'trig': 18, 'echo': 24, 'position': 'front_left_diagonal', 'angle': 315},     # Serong kiri depan (315°)
    'front_center': {'trig': 23, 'echo': 25, 'position': 'front_center', 'angle': 0},                    # Tengah depan (0°)
    'front_right_diagonal': {'trig': 12, 'echo': 16, 'position': 'front_right_diagonal', 'angle': 45}    # Serong kanan depan (45°)
}
...
class UltrasonicSensorManager:
    ...
    def __init__(self):
        ...
        self.sensor_data = {
            'front_left_diagonal': {'distance': -1, 'level': 'unknown', 'last_update': 0, 'angle': 315},
            'front_center': {'distance': -1, 'level': 'unknown', 'last_update': 0, 'angle': 0},
            'front_right_diagonal': {'distance': -1, 'level': 'unknown', 'last_update': 0, 'angle': 45}
        }
...
class UWBTracker:
    ...
    def __init__(self):
        ...
        self.bias = {
            'A0': 50.0,  # Reduced bias for better accuracy
            'A1': 50.0,
            'A2': 50.0
        }
...
class RobotController:
    ...
    def execute_obstacle_avoidance_movement(self, strategy, base_left_speed, base_right_speed):
        """
        Eksekusi pergerakan obstacle avoidance berdasarkan strategi yang diterima.
        
        Parameters:
            strategy (dict): strategi penghindaran yang dihasilkan oleh evaluate_obstacle_avoidance_strategy
            base_left_speed (float): kecepatan roda kiri sebelum modifikasi
            base_right_speed (float): kecepatan roda kanan sebelum modifikasi
        
        Returns:
            tuple: (kecepatan_roda_kiri, kecepatan_roda_kanan) sesudah penyesuaian
        """
        if not strategy or 'left_speed_factor' not in strategy or 'right_speed_factor' not in strategy:
            # Jika strategi tidak valid, kembalikan kecepatan dasar tanpa perubahan
        ...
        print(f"Executing avoidance strategy: {strategy.get('description', 'No description')}")
        print(f"Speed factors: left={left_factor}, right={right_factor}")
        print(f"Adjusted speeds from L={base_left_speed} R={base_right_speed} to L={adjusted_left_speed} R={adjusted_right_speed}")
        
        return adjusted_left_speed, adjusted_right_speed

    def smooth_speed_transition(self, target_left_speed, target_right_speed, steps=8, delay=0.04):
        """
        Smoothly transition to target wheel speeds over a few steps.
        - target_left_speed, target_right_speed: the target RPMs to reach
        - steps: number of steps in the transition
        - delay: time delay between steps (in seconds)
        """
        import time

        # Start from current speeds, get difference
        current_left = self.current_left_speed
        current_right = self.current_right_speed
        left_diff = target_left_speed - current_left
        right_diff = target_right_speed - current_right

        for i in range(1, steps + 1):
            next_left = int(current_left + (left_diff * i / steps))
            next_right = int(current_right + (right_diff * i / steps))
            self.left_motor.send_rpm(1, next_left)
            self.right_motor.send_rpm(1, next_right)
            self.current_left_speed = next_left
            self.current_right_speed = next_right
            time.sleep(delay)

        # Ensure final speeds are set
        self.left_motor.send_rpm(1, target_left_speed)
        self.right_motor.send_rpm(1, target_right_speed)
        self.current_left_speed = target_left_speed
        self.current_right_speed = target_right_speed
    ...
    def process_control_ultra_fast(self, uwb_distances, lidar, ultrasonic_manager=None):
        ...
        if lidar and hasattr(lidar, 'check_immediate_collision_threat_only'):
            if lidar.check_immediate_collision_threat_only(ultrasonic_manager, lidar):
                return

        
        # LEVEL 2: Get target direction as PRIMARY goal
        target_direction, target_distance = self.parent.uwb_tracker.estimate_target_direction(uwb_distances)
        
        # LEVEL 3: Smart obstacle avoidance
...
class FollowingRobotNode(Node):
    ...
    def __init__(self):
        ...
        # =============================================
        self.current_mode = 'otomatis'  # Default ke manual
        ...
        self.controller = RobotController("/dev/ttyRS485-1", "/dev/ttyRS485-2")
    ...
    def setup_uwb_communication(self):
        ...
        try:
            ...
            self.uwb_socket.bind((robot_ip, 5005))
            self.uwb_socket.settimeout(self.uwb_timeout)
            
            print(f"✓ UWB communication setup on {robot_ip}:5005")
